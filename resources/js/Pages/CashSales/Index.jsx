...
export default function CashSalesIndex({ transactions, date, sukis, cuts }) {
    ...
    const destroy = (id) => {
        ...
        router.delete(route('cash-sales.destroy', id));
    };

    const dayTotal = transactions.reduce((s, t) => s + Number(t.total), 0);

    return (
        <AppLayout title="Cash Sales">
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <DateNavigator date={date} routeName="cash-sales.index" />
                ...
                <Accordion key={t.id} sx={{ mb: 1 }}>
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', pr: 1 }}>
                            <Typography fontWeight={600}>{t.customer_name || 'Walk-in'}</Typography>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Chip label={peso(t.total)} size="small" color="success" />
            ...
            {transactions.length === 0 && (
                <Typography color="text.secondary" sx={{ textAlign: 'center', py: 4 }}>
                    No cash sales for this date
                </Typography>
            )}

            {transactions.length > 0 && (
                <Box sx={{ textAlign: 'right', mt: 1 }}>
                    <Chip label={`Day Total: ${peso(dayTotal)}`} color="success" />
                </Box>
            )}

            <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
                <DialogTitle>Add Cash Sale</DialogTitle>
