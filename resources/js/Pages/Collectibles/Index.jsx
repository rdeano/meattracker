...
export default function CollectiblesIndex({ transactions, date, sukis, cuts }) {
    ...
    const destroy = (id) => {
        ...
        router.delete(route('collectibles.destroy', id));
    };

    const dayTotal = transactions.reduce((s, t) => s + Number(t.total), 0);

    return (
        <AppLayout title="Collectibles (Credit Sales)">
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <DateNavigator date={date} routeName="collectibles.index" />
                ...
                <Accordion key={t.id} sx={{ mb: 1 }}>
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', pr: 1 }}>
                            <Typography fontWeight={600}>{t.customer_name || 'Unknown'}</Typography>
            ...
            {transactions.length === 0 && (
                <Typography color="text.secondary" sx={{ textAlign: 'center', py: 4 }}>
                    No collectible transactions for this date
                </Typography>
            )}
            ...
            <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
                <DialogTitle>Add Collectible (Credit Sale)</DialogTitle>
