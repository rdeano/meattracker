...
export default function CashOutIndex({ records, date, total }) {
    ...
    const destroy = (id) => {
        ...
        router.delete(route('cash-out.destroy', id));
    };

    return (
        <AppLayout title="Cash Out (Expenses)">
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <DateNavigator date={date} routeName="cash-out.index" />
            ...
            {records.length > 0 && (
                <Box sx={{ mt: 1, textAlign: 'right' }}>
                    <Chip label={`Total: ${peso(total)}`} color="error" />
                </Box>
            )}
